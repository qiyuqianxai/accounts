...
admin_user = ["jianglijuan","zhangqi","yckj1989","zhouxiang","蒋丽娟"]
def index(request):
...
def add_date_to_db(request):
    ...
    for date in all_date:
        ...
        for task_name in task_list:
            ...
            labeler_list += list(filter(lambda name:name.find("target") < 0 and name.find("system") < 0, task_db.collection_names()))
...
def get_task_done_target(request):
    ...
    for labeler in labeler_list:
        ...
        for dict in task_db[labeler].find():
            if "_id" in dict:
                dict.pop("_id")
...
def delete_task(request, dataset):
    ...
    if user not in admin_user:
        ...
        response.status_code = 403
        return response
    mongodb.drop_database(str(dataset))
