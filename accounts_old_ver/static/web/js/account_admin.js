    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(var i=0;i<g_all_date_list.length;i++)
            {
                ...
                {
                    $('#'+g_all_date_list[i]+'-task-list').append('<li><a class="task_name" title="" style="width: 280px;">'+task_list[j]+'</a></li>')
                }
            ...
            for(var k=0;k<g_all_date_list.length;k++)
            {
                $('#labeler_date_list').append('<li class="has-children">' +
                    '                        <a href="#1" title="" class="date-name" style="width: 280px">'+g_all_date_list[k]+'</a>' +
            ...
            // 加载任务完成信息单元
            $('#task_done_info').html('</li><li class="header__nav" id="task-done_date_list"></li>');
            for(var i=0;i<g_all_date_list.length;i++)
            {
                ...
                for (var j=0;j<task_list.length;j++)
                {
                    $('#'+g_all_date_list[i]+'-taskdone-list').append('<li><a class="task-done_name" title="" style="width: 280px;">'+task_list[j]+'</a></li>')
                }
            }
            ...
            // 响应任务完成情况
            $('.task-done_name').blur().on('click',function () {
                g_task_name = $(this).text();
                get_task_done_info(g_current_date+"-"+g_task_name);
                $('.task-done_name').each(function () {
                    $(this).css("color","");
                });
                $(this).css("color","red");
            })
...
// 获取对应的任务信息
function get_task_done_info(task_name) {
    jQuery(document).ajaxSend(function(event, xhr, settings) {
        function getCookie(name) {
            var cookieValue = null;
            if (document.cookie && document.cookie !== '') {
